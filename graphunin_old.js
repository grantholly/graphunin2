...
function updateURL() {
  ...
  //Set the URL parameter for server. This will also cause the page to refresh
}

function updateGraphs() {
  //var gURL = "http://" + gServer + "/render/?width=" + gWidth + "&height=" + gHeight + "&target=" + gBase + serverFromURL;
  $('#dGraphs1').append("<div class=\"metricgroup\"
  id=\"system\" >system</div>");

  $('#dGraphs2').append("<div class=\"metricgroup\">&nbsp;</div>")

  for(i in gMetrics) {
    var gURL = "http://" + gServer +
    "/render/?width=" + gWidth +
    "&height=" + gHeight +
    "&target=" + "aliasByMetric(" +
    gBase + serverFromURL + gMetrics[i][0] +
    ")&title=" + encodeURI(gMetrics[i][1]) + gMetrics[i][2];

    writeGraphs(gURL);
  }

  //Loop through file systems here
  jQuery.getJSON("http://" + gServer +
  "/metrics/find/?format=treejson&query=" +
  gBase + serverFromURL + ".df.*",

  function(mount){

    $('#dGraphs1').append("<div class=\"metricgroup\"
    id=\"filesystem\">file system</div>");
    ...
    for(x in mount){
      for(s in mountStats) {
        ...
        "&target=" + "aliasByMetric(" +
        gBase + serverFromURL + ".df." +
        mount[x].text + "." + mountStats[s][0] +
        ")&title=" + mount[x].text + encodeURI(mountStats[s][1]) +
        mountStats[s][2];
    ...
    for(x in disk){
      if(/^(sd|fio)[a-z]$|^(dm-|md)[0-9]$/.test(disk[x].text)){ //Use a regex to only render full disks and not partitions. ie sda,sdb,etc..
        for(s in diskStats) {
          ...
          "&target=" + "aliasByMetric(" +
          gBase + serverFromURL + ".disk." +
          disk[x].text + "." + diskStats[s][0] +
          ")&title=" + disk[x].text +
          encodeURI(diskStats[s][1]) + diskStats[s][2];
  ...
  jQuery.getJSON("http://" + gServer +
  "/metrics/find/?format=treejson&query=" + gBase +
  serverFromURL + ".interface.*",

  function(nics){

    $('#dGraphs1').append("<div class=\"metricgroup\"
    id=\"networks\">networks</div>");
    ...
    for(x in nics){
      for(s in nicStats) {
        ...
        "&height=" + gHeight +
        "&target=" + "aliasByMetric(" +
        gBase + serverFromURL + ".interface." +
        nics[x].text + "." + nicStats[s][0] +
        ")&title=" + nics[x].text +
        encodeURI(nicStats[s][1]) + nicStats[s][2];
  ...
  function(jvms){
    ...
    for(x in jvmInstances){
      ...
      $('#jvms').append("<a href=\"#jvm" + jvmInstances[x] +
      "\">jvm-" + jvmInstances[x] + "</a><BR>");

      $('#dGraphs1').append("<div class=\"metricgroup\"
      id=\"jvm" + jvmInstances[x] + "\">jvm-" +
      jvmInstances[x] + "</div>");
      ...
      for(s in jvmStats) {
        var gURL = "http://" + gServer +
        "/render/?width=" + gWidth +
        "&height=" + gHeight + "&target=" +
        "aliasByMetric(" + gBase + serverFromURL + ".GenericJMX." +
        jvmInstances[x] + "-" + jvmStats[s][0] + ")&title=" +
        jvmInstances[x] + encodeURI(jvmStats[s][1]) + jvmStats[s][2];
...
function writeGraphs(url){
  ...
  $('#dGraphs1').append("<a href=\"graph-zoom.html?" +
  url.split("?")[1] + "&server=" + serverFromURL +
  "\" target=\"_blank\"> <img src=" + url +
  "&from=" + gC1From + "> </a><BR>");

  $('#dGraphs2').append("<a href=\"graph-zoom.html?" +
  url.split("?")[1] + "&server=" + serverFromURL +
  "\" target=\"_blank\"> <img src=" + url +
  "&from=" + gC2From + "> </a><BR>");
